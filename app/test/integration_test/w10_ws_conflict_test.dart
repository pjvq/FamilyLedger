...
void main() {
  ...
  group('W10 WebSocket Connection E2E', () {
    ...
    test(
      'WS-003: Connect without token → auth timeout closes connection',
      () async {
  ...
  group('W10 Family Broadcast E2E', () {
    ...
    test('FB-001: Register owner + 2 members', () async {
      ...
      final cResp = await authClient.register(
        auth_pb.RegisterRequest()
    ...
    test(
      'FB-003: Owner pushes family op → B and C both receive WS notification',
      () async {
  ...
  group('W10 Conflict Resolution (LWW) E2E', () {
    ...
    test('LWW-001: Register two family members for conflict tests', () async {
      ...
      await familyClient.joinFamily(
        family_pb.JoinFamilyRequest()..inviteCode = inviteResp.inviteCode,
        options: CallOptions(metadata: {'authorization': 'Bearer $userBToken'}),
      );
    });

    test(
      'LWW-002: Two clients concurrent Push → LWW → both Pull consistent',
      () async {
        final entityId = _uuid();
        ...
        expect(
          updatesA.last.payload,
          equals(updatesB.last.payload),
  ...
  group('W10 WebSocket Heartbeat E2E', () {
    ...
    test(
      'HB-002: Connection stays alive for 3s (server sends pings)',
      () async {
        final ws = await _connectWs(harness, userToken);
    ...
    test('HB-004: All connections receive notification on push', () async {
      ...
      ws1.listen((m) {
        msgs1.add(m as String);
        if (!c1.isCompleted) c1.complete();
      });
      ws2.listen((m) {
        msgs2.add(m as String);
  ...
  group('W10 WebSocket Message Size E2E', () {
    ...
    test('MS-002: Send >512 bytes → server closes connection', () async {
      ...
      await closeCompleter.future.timeout(
        const Duration(seconds: 5),
        onTimeout: () =>
            fail('Server did not close connection on oversized message'),
      );
  ...
  group('W10 Token Expiry E2E', () {
    ...
    String makeShortLivedJwt(String userId, {int ttlSeconds = 2}) {
      ...
      final signature = base64Url.encode(sig.bytes).replaceAll('=', '');

      return '$header.$payload.$signature';
    }

    test('TE-001: Short-lived token connects successfully while valid', () async {
    ...
    test(
      'TE-002: Short-lived token → server closes with code 4001 after expiry',
      () async {
        // Register user
        final resp = await authClient.register(
          auth_pb.RegisterRequest()
    ...
    test(
      'TE-003: After token-expiry kick → refresh → reconnect succeeds',
      () async {
        final resp = await authClient.register(
          auth_pb.RegisterRequest()
  ...
  group('W10 Pong Timeout E2E', () {
    test(
      'PT-001: Connection without pong → server closes after pongWait',
      () async {
        final resp = await authClient.register(
          auth_pb.RegisterRequest()

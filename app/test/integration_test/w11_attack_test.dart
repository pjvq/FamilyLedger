...
void main() {
  ...
  group('DATA ISOLATION', () {
    ...
    test('Setup: Two independent users with transactions', () async {
      // Register user A
      final respA = await authClient.register(
      ...
      // Register user B
      final respB = await authClient.register(
        auth_pb.RegisterRequest()
      ...
      try {
        ...
        expect(
          e.code == StatusCode.permissionDenied ||
              e.code == StatusCode.notFound,
          isTrue,
          reason: 'Should reject B modifying A\'s transaction, got: ${e.code}',
        );
      }
    });

    test(
      'ISOLATION-003: User B cannot create transaction on A\'s account',
      () async {
        final bOpts = CallOptions(
          metadata: {'authorization': 'Bearer $userBToken'},
        );
    ...
    test('ISOLATION-004: User B cannot list A\'s transactions', () async {
      ...
      final leakedTxns = listResp.transactions.where(
        (t) => t.note.contains('Secret purchase A'),
      );
      expect(
        leakedTxns,
        isEmpty,
  ...
  group('BALANCE INTEGRITY', () {
    ...
    test('Setup: Fresh user with known balance', () async {
      ...
      final opts = CallOptions(metadata: {'authorization': 'Bearer $token'});
    ...
    test(
      'BALANCE-001: Expense exceeding balance (overdraft allowed?)',
      () async {
        final opts = CallOptions(metadata: {'authorization': 'Bearer $token'});
        ...
        try {
          await txnClient.createTransaction(
            txn_pb.CreateTransactionRequest()
              ...
              ..categoryId = categoryId
              ..amount =
                  Int64(50000) // 500 CNY, balance is only 100 CNY
              ..currency = 'CNY'
          ...
          expect(
            ...
            reason:
                'Cash account should reject overdraft with FailedPrecondition',
          );
        }
      },
    );
    ...
    test('BALANCE-002: INT64_MAX amount (overflow attack)', () async {
      ...
      try {
        ...
        expect(
          ...
          reason:
              'Amount exceeding 10 billion should be rejected with InvalidArgument',
        );
      }
    });
  ...
  group('SYNC INCONSISTENCY', () {
    ...
    test('SYNC-003: Push with duplicate clientId is idempotent', () async {
      ...
      final matches = acctResp.accounts.where(
  ...
  group('PERMISSION BYPASS', () {
    ...
    test('PERM-001: Member cannot edit own transaction (no can_edit)', () async {
      final memberOpts = CallOptions(
        metadata: {'authorization': 'Bearer $memberToken'},
      );
      ...
      try {
        ...
        fail(
          'BUG: Member without can_edit was able to edit transaction on family account!',
        );
      } on GrpcError catch (e) {
        expect(
          e.code,
          equals(StatusCode.permissionDenied),
          reason:
              'Member without can_edit should be denied even for own transactions on family accounts',
        );
      }
    });

    test(
      'PERM-002: Member cannot delete own transaction (no can_delete)',
      () async {
        final memberOpts = CallOptions(
          metadata: {'authorization': 'Bearer $memberToken'},
        );
        ...
        try {
          ...
          fail(
            'BUG: Member without can_delete was able to delete transaction on family account!',
          );
        } on GrpcError catch (e) {
          expect(
            e.code,
            equals(StatusCode.permissionDenied),
            reason:
                'Member without can_delete should be denied even for own transactions on family accounts',
          );
        }
      },
    );
    ...
    test(
      'PERM-003: Member cannot create family account (no can_manage_accounts)',
      () async {
        final memberOpts = CallOptions(
          metadata: {'authorization': 'Bearer $memberToken'},
        );
        ...
        try {
          ...
          fail(
            'BUG: Member without can_manage_accounts created a family account!',
          );
        } on GrpcError catch (e) {
          expect(
            e.code,
    ...
    test(
      'PERM-004: Member cannot set own permissions (privilege escalation)',
      () async {
        final memberOpts = CallOptions(
          metadata: {'authorization': 'Bearer $memberToken'},
        );
      ...
      try {
        await txnClient.createTransaction(
          txn_pb.CreateTransactionRequest()
            ..accountId = accountId
            ..categoryId =
                _uuid() // random non-existent category
            ..amount = Int64(100)
            ..currency = 'CNY'
            ..amountCny = Int64(100)
            ..exchangeRate = 1.0
    ...
    test('INPUT-005: Extremely long note (10KB)', () async {
      ...
      try {
        ...
        expect(
          ...
          equals(StatusCode.invalidArgument),
          reason:
              'Notes exceeding 1000 chars should be rejected with InvalidArgument',
        );
      }
    });
      ...
      try {
        await txnClient.createTransaction(
          txn_pb.CreateTransactionRequest()
            ...
            ..currency = 'USD'
            ..amountCny =
                Int64(0) // missing CNY amount!
            ..exchangeRate = 7.2
            ..type = txn_enum.TransactionType.TRANSACTION_TYPE_EXPENSE
            ..note = 'USD without CNY conversion',
          options: opts,
        );
        fail('BUG: Foreign currency transaction accepted without amountCny!');
      } on GrpcError catch (e) {
        expect(
          e.code,
          equals(StatusCode.invalidArgument),

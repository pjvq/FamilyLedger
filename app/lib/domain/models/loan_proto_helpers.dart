...
String loanTypeToString(pb_enum.LoanType type) => _loanTypeMap[type] ?? 'other';

String repaymentMethodToString(pb_enum.RepaymentMethod method) =>
    _repaymentMethodMap[method] ?? 'equal_installment';

String subTypeToString(pb_enum.LoanSubType type) => _subTypeMap[type] ?? '';

String rateTypeToString(pb_enum.RateType type) => _rateTypeMap[type] ?? 'fixed';

// Reverse lookups (string → proto)
final _loanTypeReverse = {for (final e in _loanTypeMap.entries) e.value: e.key};
final _repaymentMethodReverse = {
  for (final e in _repaymentMethodMap.entries) e.value: e.key,
};
final _interestCalcMethodReverse = {
  for (final e in _interestCalcMethodMap.entries) e.value: e.key,
};
...
pb_enum.RepaymentMethod stringToRepaymentMethod(String method) =>
    _repaymentMethodReverse[method] ??
...
pb_enum.InterestCalcMethod stringToInterestCalcMethod(String method) =>
    _interestCalcMethodReverse[method] ??
    pb_enum.InterestCalcMethod.INTEREST_CALC_MONTHLY;

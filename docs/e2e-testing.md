...
**总计**: 330 Go + 535 Widget + 6 Perf + 24 Shell + 79 Integration + 6 脚本 = **~980 测试点**
...
使用 `pgxmock/v4` mock 数据库 + `testify` 断言。所有 service 已重构为 `db.Pool` 接口（`server/pkg/db/pool.go`），支持 mock 注入。

> **2026-04-27 更新**: 新增 4 个 package（`pkg/audit`、`pkg/config`、`pkg/storage`、`pkg/ws`），总计 18 个 test package。`pkg/ws` 包含真实 Ping 超时测试（90s）。
...
| **export** | **14** | CSV/Excel/PDF 三格式 + 全量备份, 日期校验, 金额转换 |
| **pkg/audit** | **5** | 审计日志 helper + DB 写入 |
| **pkg/config** | **11** | JWT Secret 校验 (生产/开发/长度/缺失) |
| **pkg/storage** | **9** | FileStorage 接口 (Local + S3 placeholder) |
| **pkg/ws** | **7** | WebSocket Ping-Pong 心跳, 广播, 超时断连 (90s) |
| **合计** | **~330** | **18 个 package 全覆盖** |
...
| `sync_engine_lww_test.dart` | SyncEngine LWW 冲突解决 |
| `transaction_add_no_flicker_test.dart` | Server-first ID 无闪烁 |
| `transaction_provider_load_test.dart` | 分页加载 (pageSize=100) |
...
```bash
# 1. Go 单元测试（不需要数据库）
cd server && go test ./... -count=1          # 330 tests, ~94s (ws package has 90s timeout test)

# 2. Flutter Widget 测试（不需要后端）
cd app && flutter test --reporter compact    # 535 tests, ~12s

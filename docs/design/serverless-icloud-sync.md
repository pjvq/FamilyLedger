...
| **iOS 家庭共享** | ✅ **CKShare（仅 iOS，用户可选用）** | 仅 Apple 设备间；**放弃服务端强制的细粒度 5 维权限**，采用「共享即信任」（见 §8）。 |
...
**一句话**：两端都变成**本地优先、零服务器**；iOS 额外用 iCloud 做多端同步、并支持用户自行开启的家庭共享，Android 是单机本地账本。架构更简单、更私密；代价是 **(1) iOS↔Android 不互通**、**(2) Android 无自动多端同步（靠本地备份兜底）**、**(3) 家庭权限采用「共享即信任」**。
...
| 定时任务（预算检查、贷款提醒、折旧、汇率刷新） | `server/cmd/server/main.go` | ✅ **本地化**：纯计算 + 本地通知（iOS `UNNotificationRequest`）。**MINOR5**：Background App Refresh 不保证触发时机，**固定日期类提醒（如贷款还款日、账单日）改用 `UNCalendarNotificationTrigger` 预排兜底**，不依赖 BAR |
...
> ⚠️ Flutter 接入 CloudKit 无成熟一等公民插件。**已定**：通过自研 **Platform Channel 调原生 Swift CloudKit**（不依赖能力有限的社区插件）。这是 iOS 原生工作量，计入 Phase 3。
...
（同步实体清单见 `sync_engine.dart:72-82` 的 `_knownEntityTypes`，共 9 项。）

> **`category_merge` 不是领域实体，不建分类记录类型。** 它是「把分类 A 合并进 B」的一次性**操作**。CloudKit 是状态模型而非操作日志，因此合并应在本地展开为：受影响交易的 `category_id` 改写（普通 update）+ 源分类软删除（tombstone），这些普通变更照常同步即可。`category_merge` 本身不上云。

### 6.2 必须补齐的缺口

1. **删除语义不统一（tombstone）**
   - 现状（按**可同步领域实体**口径，非全部 15 张表）：8 个可同步实体中，6 个有 `deleted_at`（transaction / category / loan / loan_group / investment / fixed_asset）；**account 用 `isActive=false` 软删**（语义等价、结构不同）；**budget 是硬删除**（无 tombstone）。
   - CloudKit：删除是 `CKRecord` 的真实删除，靠 `CKFetchRecordZoneChangesOperation` 的 `deletedRecordZoneIDs` 回传。
   - **方案**：统一为「软删除 + tombstone」或依赖 CloudKit 删除变更回传；**重点补齐 budget 的删除传播**，并把 account 的 `isActive` 映射为删除语义。

2. **缺少逐行版本号 / 向量时钟**
   - 现状：只有 `updated_at`（约一半表有）+ 出站队列，**无 version、无 vector clock、无 CRDT**。
   - CloudKit：自带 `recordChangeTag`，且服务端做逐字段 LWW。
   - **方案**：保留现有「客户端时间戳 LWW」语义，用 CloudKit 的 `recordChangeTag` 做乐观并发；冲突时按 `updated_at` 取舍（与现状一致）。**另见 §7.3：冲突窗口放大后建议引入 Lamport/HLC。**

3. **图片 blob → `CKAsset`（含配额策略）**
   - 现状：图片**不在 SQLite**，存在服务端文件系统（`storage.go`，S3 是未实现的桩 `s3.go:25-26`），客户端只存 URL（`core_tables.dart:64`，逗号分隔）；服务端现有上限 5MB/张、500 张/用户（`transaction/service.go`）。
   - **方案**：迁移为 **`CKAsset`** 挂在对应 transaction 记录；本地缓存文件，URL 字段改为本地/CloudKit 资产引用。
   - **配额策略（重要，M1）**：iCloud 免费仅 5GB 且与照片/备份/邮件**共享**，票据图极易吃满。需定义：
     - **上传前压缩**（如长边 ≤1600px、JPEG q≈0.7，单张目标 <300KB）；
     - 保留「原图/压缩」开关，默认压缩；
     - **配额满时的降级行为**：图片同步失败不阻塞数据同步，单独排队重试 + 明确提示用户；
     - **不做云端自动清理**（首版）：仅提供手动删除，避免误删；老图保留本地。
...
| `PullChanges`（`since` 时间水位 + 分页游标） | `CKFetchRecordZoneChangesOperation` + **`serverChangeToken`** 游标 |
| WebSocket 实时推送 | `CKSubscription`（数据库订阅）+ 静默推送，收到后触发增量拉取 |
| 30s 轮询兜底（`AppConstants.syncIntervalSeconds`） | 保留为兜底；CloudKit 推送为主 |

> **M5 — 游标不是时间戳，是不透明 blob**：CloudKit 的 `serverChangeToken` 是二进制 `Data`，**不能**直接塞进现有 `SyncMetadata.value`（`int` 类型，存 `sync_last_pull_ts`）。需要 schema 改动：新增一列（`TextColumn`，存 Base64）或新建专用表存 token。这不是「换个写入目标」那么简单。

### 7.2 出站流程（机制复用，但 payload 要重做）

现有 `SyncQueue`（`support_tables.dart:67-81`，含 `uploaded/retryCount/nextRetryAt`）、死信队列（`SyncDeadLetters`）的**重试/幂等/死信机制可复用**。但：

- **payload 序列化要改（MINOR2）**：`SyncQueue.payload` 目前存 gRPC protobuf 序列化结果，换 CloudKit 后需改为 CKRecord 可接受的字段/类型映射（native types / JSON），序列化层要重写。
- **出站需按「操作组」分批（R1，见下）**，而非逐条独立上送。

> **R1 — CloudKit 跨记录原子性（已定）**：`CKModifyRecordsOperation` 的原子性是 **per-zone per-operation**。一笔转账涉及 2 条 transaction + 2 个 account 余额变更，必须放进**同一个 operation** 才原子（当前由 PG 事务保证）。**已定**：出站以「**操作组**」为单位 —— 同一本地写事务内产生的 ops 聚成一个 `CKModifyRecordsOperation` 原子提交，而非逐条上送。

### 7.3 冲突解决（保留语义；HLC 列入 Phase 3 范围）

- 继续沿用**客户端时间戳 LWW**（与 `sync_engine.dart:567-613` 一致）。
- **已知风险**：设备时钟漂移时较旧写可能静默覆盖较新写。当前服务端版本同样如此（采纳客户端时间戳），**非去服务器新增缺陷**。
- **R2 — 冲突窗口放大（已定）**：gRPC + WebSocket 的冲突窗口约毫秒级；CloudKit 静默推送 + 30s 轮询兜底后可能放大到**分钟级**，纯 wall-clock 时间戳 LWW 误判概率上升。**已定**：Phase 3 引入 **HLC（混合逻辑时钟）**，每条记录带 `hlc` 字段、冲突按 hlc 排序，取代纯墙钟 LWW。
...
- 邀请：用 CloudKit `CKShare` 的系统级共享邀请（`UICloudSharingController`）替代自建邀请码。
  - **MINOR3**：`UICloudSharingController` 是 UIKit API，Flutter 中需经 **Platform Channel** 调起原生分享 UI。
...
| 平台 | 存储 | 同步 | 家庭共享 |
|------|------|------|---------|
| **iOS / macOS** | 本地 Drift（事实来源） | ✅ iCloud（CloudKit）多端同步（§5–§7） | ✅ CKShare（§8，用户可选用，仅 Apple 设备间） |
...
- `SyncQueue` / `SyncMetadata` / 死信队列是为同步服务的（**MINOR4**）：Android 备份是「整库导出加密文件」（§9.2），是**全量快照**而非增量，因此**不需要**这些表；在 Android 构建中编译期关闭即可。若未来要做「增量备份」再考虑保留。
...
**加密细节（已定）**：备份文件用 **AES-256-GCM** 加密，密钥由用户口令经 **Argon2id** 派生；文件格式 = `magic header + 版本号 + salt + nonce + 密文`，头部记录 `schemaVersion` 以便跨版本恢复。


### 9.3 一份代码两种形态

Flutter 单 codebase，用编译期开关 / 平台判断区分：iOS 走「Drift + iCloudSyncEngine」，Android 走「Drift only + 手动加密备份」。同步层做成可插拔（接口 + 平台实现），避免在 Android 构建里引入 CloudKit/原生依赖。
...
| Flutter 无成熟 CloudKit 插件 | 🟡 中 | 自研原生 Swift Platform Channel（§4，已定），计入 Phase 3 |
| 时钟漂移导致 LWW 误判 | 🟢 低（现状已有） | Phase 3 引入 HLC（§7.3，已定） |
...
架构评审提出的实现细节，已逐项拍定默认方案（如实施中发现更优解可再调，但默认即下列值）：

| 编号 | 事项 | **已定方案** | 章节 |
|------|------|-------------|------|
| **M1** | 图片配额策略 | 上传前压缩：长边 ≤1600px、JPEG q≈0.7、单张目标 <300KB；配额满时图片进**独立重试队列、不阻塞数据同步**并提示用户；首版**不做云端自动清理**（提供手动删除） | §6.2(3) |
| **M2** | 首次全量上云 | 分批 **200 records/批**（留 CloudKit 400 上限余量），CKAsset 单独上传；用**迁移进度表记录已传 `recordName`** 实现断点续传，失败只补未完成项 | §10 |
| **M3** | `category_merge` | **不上云**；本地展开为「交易 `category_id` 改写（update）+ 源分类 tombstone」，按普通变更同步 | §6.1 |
| **M4** | tombstone 补齐 | 给 **budget 增 `deleted_at` 软删列**；**account 的 `isActive=false` 映射为删除语义**；两者纳入删除变更传播 | §6.2(1) |
| **M5** | 游标存储 | **新建 `CloudKitSyncState(zone TEXT PK, change_token TEXT)` 表**存 Base64 token，不复用 `SyncMetadata`（`int`） | §7.1 |
| **R1** | 跨记录原子性 | **操作组** = 同一本地写事务内产生的 ops（转账 = 2 txn + 2 account 共 4 条）聚成**一个 `CKModifyRecordsOperation`** 原子提交；出站按操作组而非逐条 | §7.2 |
| **R2** | 冲突排序 | **Phase 3 引入 HLC（混合逻辑时钟）**：每条记录带 `hlc` 字段，冲突按 hlc 排序（取代纯 wall-clock LWW） | §7.3 |

> 以上均为 **Phase 3（iCloud 同步引擎）** 的实现细节，**不阻塞 Phase 1/2**（服务端下沉、Android 本地版）。

...
> **R1 — CloudKit 跨记录原子性（实施期需出 ADR）**：`CKModifyRecordsOperation` 的原子性是 **per-zone per-operation**。一笔转账涉及 2 条 transaction + 2 个 account 余额变更，必须放进**同一个 operation** 才原子；当前是 PG 事务保证。因此出站逻辑要把相关变更聚成「操作组」一起提交。**Phase 3 开工前先出 ADR 定义操作组粒度。**
...
- **R2 — 冲突窗口放大**：gRPC + WebSocket 的冲突窗口约毫秒级；CloudKit 静默推送 + 30s 轮询兜底后可能放大到**分钟级**，纯时间戳 LWW 误判概率上升。因此**将「至少引入 Lamport 计数器（理想 HLC 混合逻辑时钟）」列为 Phase 3 的 scope-in，而非"后续可选"**。
...
## 13b. Phase 3（iCloud 同步引擎）开工前的 TODO 清单

架构评审提出、需在写同步代码前定清楚的事项（来源：PR #138 review）：

| 编号 | 事项 | 说明 / 决策方向 | 章节 |
|------|------|----------------|------|
| **M1** | 图片配额策略 | iCloud 免费 5GB 共享，需压缩上限 + 配额满降级 + 清理策略 | §6.2(3) |
| **M2** | 首次全量上云量化 | `CKModifyRecordsOperation` ≤400 records/次、CKAsset ≤50MB；1 万行 + 图片约 25–50 批；需断点续传 | §10 |
| **M3** | `category_merge` 处理 | 不上云；本地展开为 update + tombstone | §6.1 |
| **M4** | tombstone 补齐 | budget 硬删除需补删除传播；account 的 `isActive` 映射删除语义 | §6.2(1) |
| **M5** | 游标存储改造 | `serverChangeToken` 是 blob，非 `int`；需新列/新表（Base64） | §7.1 |
| **R1** | 跨记录原子性 ADR | 转账等多记录变更聚成「操作组」单 operation 提交 | §7.2 |
| **R2** | 冲突排序增强 | 冲突窗口放大 → Lamport/HLC 列入 Phase 3 范围 | §7.3 |

> 这些**不阻塞 Phase 1/2**（服务端下沉、Android 本地版），仅是 **Phase 3 开工前**要收口的设计点。

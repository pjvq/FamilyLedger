...
>
> **最终决策（2026-06-23）**：采用**平台分化**策略 ——
> **iOS / macOS：本地 Drift + iCloud（CloudKit）多端同步**；
> **Android：纯本地 Drift，单机保存，不参与同步**；
...
### 9.2 Android 必须补的一条：本地备份 / 设备迁移（防丢数据）

「只本地保存」最大的风险是**换机/丢机 = 数据全丢**。即使不做同步，也**强烈建议**给 Android 一条手动备份/恢复通道（这不是「同步」，是「备份」）：

| 选项 | 说明 | 推荐度 |
|------|------|--------|
| **手动导出/导入文件** | 导出整库为加密文件（或复用现有 CSV 导出，`docs/import-export-design.md`），用户自行存到任意位置，换机时导入 | ✅ 最简单，零依赖 |
| **Google Drive `appDataFolder` 备份** | 把加密备份**文件**丢进 App 私有文件夹（非同步，仅整文件备份/恢复） | 🟡 自动化更好，但要接 Drive API |
| **Android Auto Backup** | 系统级自动备份 SQLite 到用户 Google 账号 | 🟡 透明但不可控、跨设备恢复时机有限 |

> 注意：这只是**单向备份/恢复**，不是多端实时同步；不引入 CRDT、不跨平台。与「Android 只本地保存」的决策一致。
...
3. **首次上云**：`iCloudSyncEngine` 把本地全量映射为 CKRecord 批量写入用户私有库（注意 CloudKit 批量限额，分批）。
4. **图片迁移**：旧服务端文件系统中的票据图 → 下载 → 作为 `CKAsset` 重新上云。
5. **双跑期（可选）**：提供一个过渡版本同时支持「旧 gRPC」与「iCloud」，验证一致性后切换。
6. **退役服务端**。
...
| 微信登录丢失 | 🟡 中 | 改 Apple 登录 / iCloud 账号身份 |
...
- **Phase 5 — 迁移与退役**：数据迁移工具、双跑验证、退役 Go 服务端 + PostgreSQL。

---

## 14. 待决问题（Open Questions）

1. ✅ **跨平台已定**：iOS 用 iCloud、Android 仅本地、不互通。（§9）
2. **Android 备份形态**：手动导出文件 / Drive `appDataFolder` / 系统 Auto Backup，选哪条？备份是否加密、密钥如何管理？（§9.2）
3. **家庭权限**：能否接受「共享即信任」（仅 iOS）？还是家庭共享暂不做？（§8）
4. **微信登录**：是否有不可放弃的微信登录用户群？
5. **存量用户**：当前有多少真实用户/多少数据需要迁移？双跑期是否必要？
6. **macOS**：是否要同时支持 macOS 客户端（CloudKit 同套方案天然支持）？

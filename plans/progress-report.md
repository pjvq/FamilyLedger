...
> 截止 2026-04-24 17:50 | 148 commits | monorepo: proto/ + server/ (Go) + app/ (Flutter)
> 基于全量代码审查 | Go 242 tests + Flutter 566 tests = **808 全绿**
...
| Phase 8: 多币种 + CSV导入 + OAuth | ⚠️ | ⚠️ | **50%** | 汇率 ✅; OAuth ❌ mock |
...
| Dart generated (proto + drift) | ~20,300 行 |
...
│   ├── asset/service.go        — 805 行, 13 方法, auth ✅, tx ✅, permission ✅ (f0ecf36)
│   ├── auth/service.go         — 274 行, 4 方法, JWT + bcrypt
│   ├── budget/service.go       — 479 行, 9 方法, auth ✅, tx ✅, permission ✅ (f0ecf36)
...
│   ├── investment/service.go   — 645 行, 9 方法, auth ✅, FOR UPDATE ✅, permission ✅ (f0ecf36)
│   ├── loan/service.go         — 1,583 行, 18 方法, auth ✅, tx ✅, permission ✅ (f0ecf36)
...
| family | 665 | 11 | **17** | 8 | - | 2 | - |
| sync | 787 | 14 | **6** | 2 | - | 1 | 3 |
| budget | 479 | 9 | **13** | 6 | ✅ | 2 | - |
| notify | 541 | 11 | **22** | 6 | - | - | - |
| loan | 1,583 | 18 | **34** | 13 | ✅ | 4 | - |
| investment | 645 | 9 | **29** | 8 | ✅ | 1 | 1 |
| asset | 805 | 13 | **31** | 9 | ✅ | 3 | - |
| market | 424 | 6 | **17** | - | - | - | - |
| exchange | 230 | - | 0 | - | - | - | - |
| dashboard | 707 | 6 | **14** | 5 | - | - | - |
| export | 330 | 2 | **14** | 1 | 1 | - | - |
| importcsv | 445 | 3 | **16** | 1 | - | 1 | - |
| **合计** | **~9,600** | **125** | **242** | **73** | **17** | **22** | **9** |

### 测试覆盖

- **全部覆盖 (14/15)**: auth(14), transaction(11), account(10), budget(13), dashboard(14), sync(6), loan(34), asset(31), investment(29), family(17), notify(22), importcsv(16), market(17), export(14)
- **无独立测试 (1/15)**: exchange（内嵌在 market 服务中，无独立 RPC）
...
| budget | StateNotifier | ✅ | ✅ | ✅ (f0ecf36) |
| dashboard | StateNotifier | ✅ local-first | ✅ 3s timeout | ❌ |
| family | StateNotifier | ✅ | ✅ | ✅ (核心) |
| loan | StateNotifier | ✅ | ✅ | ✅ (f0ecf36) |
| investment | StateNotifier | ✅ | ✅ | ✅ (f0ecf36) |
| asset | StateNotifier | ✅ | ✅ | ✅ (f0ecf36) |
...
| # | 问题 | 影响 | 状态 |
|---|------|------|------|
| 3 | ~~budget/loan/asset/investment 缺 family permission 检查~~ | ~~家庭模式下无权限控制~~ | ✅ 已修复 `f0ecf36` |
| 4 | ~~Go 后端 9/15 service 零测试~~ | ~~loan(1583行)无任何测试~~ | ✅ 已修复 `7ae8578` (242 tests) |
| 5 | 家庭协作未做多用户端到端验证 | 未知兼容性问题 | 未开始 |
| 6 | loan service 无 FOR UPDATE | 并发还款可能余额 race | 未开始 |

### 🟢 低优先级

| # | 问题 | 状态 |
|---|------|------|
| 7 | ~~批量删除交易（proto 无此 RPC）~~ | ✅ 已修复 `585b133` |
| 8 | ~~交易图片附件未验证实际上传~~ | ✅ 已修复 `2bcaa1f` + `421935d` |
| 9 | 60fps 实机性能验证 | 未开始 |
| 10 | 信用卡账单日提醒逻辑缺失 | 未开始 |
...
| 层级 | 数量 | 覆盖 |
|------|------|------|
| Go unit tests | **242** | **14/15 service** |
...
| ~~**P3**~~ | ~~budget/loan/asset/investment family 权限~~ | ~~1 天~~ | ✅ `f0ecf36` |
| ~~**P3**~~ | ~~Go 后端剩余 9 service 测试~~ | ~~2-3 天~~ | ✅ `7ae8578` |
| **P3** | 家庭协作多用户 E2E | 1 天 | 未开始 |
| **P3** | 图片附件真机验证 | 0.5 天 | 未开始 |
| **P3** | 60fps 实机性能验证 | 0.5 天 | 未开始 |
| **P3** | 图片存储 → 对象存储 | 1 天 | 未开始 |
| **P3** | 清理调试 print 语句 | 0.5 天 | 未开始 |

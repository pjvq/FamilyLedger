...
1. As a 用户, I want to 创建自定义主分类, so that 适应我的个人需求
...
1. As a 用户, I want to 预设分类带有子分类（如餐饮下有早餐、午餐、晚餐、夜孜）, so that 记账时可以更精细地归类
1. As a 用户, I want to 创建自定义主分类, so that 系统预设分类不满足时可以自己扩展
1. As a 用户, I want to 在任何主分类下添加自定义子分类, so that 更精细地跟踪某类支出的去向
1. As a 用户, I want to 编辑分类名称和图标, so that 调整分类的展示效果
1. As a 用户, I want to 删除自定义分类（预设分类不可删）, so that 清理不再使用的分类
1. As a 用户, I want to 调整分类和子分类的排序顺序, so that 常用分类排在前面
1. As a 用户, I want to 主分类和子分类都可以从内置图标库中选择图标（不需要上传）, so that 保持视觉一致性
1. As a 用户, I want to 记账时先选主分类再选子分类（子分类可选）, so that 快速记账时只选主分类即可，精细记账时可以再选子分类
1. As a 用户, I want to 报表和预算能按子分类统计, so that 知道“餐饮”里多少花在了外卖、多少花在了餐厅

#### 分类体系设计

**层级结构：**主分类 → 子分类（两级，不支持更深层级）

**预设分类（带子分类）示例：**

| 主分类 | 图标 | 预设子分类 |
|--------|------|------------|
| 餐饮 | 🍜 | 早餐、午餐、晚餐、夜孜、饮品、水果零食 |
| 交通 | 🚗 | 地铁公交、打车、加油、停车 |
| 购物 | 🛍️ | 电器数码、日用百货、美妆护肤 |
| 居住 | 🏠 | 房租、物业、水电燃气、家政服务 |
| 娱乐 | 🎮 | 电影演出、游戏、运动健身、书籍 |
...
- App 内置 ~60 个矢量图标（SF Symbols / Material Icons 风格）
- 分类：餐饮、交通、购物、居住、娱乐、健康、教育、工作、金融、其他
- 用户不需要上传图标，只从图标库中选择
- 图标存储为字符串 key（如 `food_breakfast`、`transport_metro`），客户端根据 key 渲染对应图标

**记账流程变化：**

```
旧流程: 金额 → 选主分类 → 确认
新流程: 金额 → 选主分类 → [可选] 选子分类 → 确认
```

- 选子分类是可选的，不选则归入主分类
- 有子分类的主分类点击后展开子分类面板（动画滑入）
- 无子分类的主分类直接选中

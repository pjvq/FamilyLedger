...
### Phase 4b: 组合贷款增强 (NEW)
- [ ] 支持三种房贷形式：纯商贷 / 纯公积金贷 / 组合贷款
- [ ] 组合贷 = 独立的商贷 + 公积金贷，各自利率、各自还款计划
- [ ] 利率类型：固定利率 / LPR浮动（base + spread，年度调整）
- [ ] LPR 利率调整月可选（每年1月 / 放款对应月）
- [ ] 组合贷提前还款可指定先还哪部分（默认利率高的）
- [ ] 贷款详情页 Tab 视图：总览 / 商贷 / 公积金
- [ ] 列表页组合贷卡片：分段进度条 + 分拆月供展示
- [ ] 向后兼容：现有独立贷款不受影响

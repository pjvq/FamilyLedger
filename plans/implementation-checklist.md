...
> Last updated: 2026-04-24 17:43 (148 commits)
...
- [x] **细粒度权限** - ✅ pkg/permission 权限检查包 + 后端强制 + 前端 UI 门控 (`c4b819f`)
- [x] **个人/家庭账本切换** - ✅ `107ab8e` 前端切换器+后端数据隔离
- [x] 添加多个资金账户(7 种类型)
- [x] 账户间转账不影响总额
...
关键 commits:
- `3b61a4b` 初始 68 Go tests (account, auth, budget, dashboard, sync, transaction)
- `fa70666` loan 34 tests (638 行)
- `7ae8578` 剩余 7 service 140 tests (asset, investment, family, notify, importcsv, market, export)
...
| # | 问题 | Commit |
|---|------|--------|
| 1 | Go 后端零单元测试 → **242 tests, 14 service 全覆盖** | `3b61a4b` → `7ae8578` |
...
| 13 | 细粒度权限 + 双账本 | `c4b819f` + `107ab8e` |
| 14 | 子分类聚合统计 | `98f7038` |
| 15 | SyncService 分类操作同步 | `0a586a6` |
| 16 | budget/loan/asset/investment 家庭权限 | `f0ecf36` |
| 17 | 批量删除交易 | `585b133` |
| 18 | 交易图片附件 + 安全加固 | `2bcaa1f` + `421935d` |
...
| 3 | **家庭协作 E2E 验证** | P3 | 0.5 天 |
| 4 | **信用卡账单日提醒** | P3 | 0.5 天 |
| 5 | **图片附件真机验证** | P3 | 0.5 天 |
| 6 | **60fps 实机验证** | P3 | 0.5 天 |
| 7 | **图片存储 → 对象存储** | P3 | 1 天 |
| 8 | **清理调试 print 语句** | P3 | 0.5 天 |

...
## Phase 8: 多币种 + 智能导入导出 + OAuth 登录

> 🌍 **User Stories**: #2 (OAuth), #14, #15, #18, #19, #20
> 多币种自动汇率 → 智能导入(支付宝/微信/通用CSV) → 导出优化 → 微信/Apple 登录
...
- ImportService 扩展:
  - 支付宝账单解析：跳过前 4 行头部信息、GBK 编码、列名映射（交易时间/商品名称/金额/收支/交易状态）
  - 微信账单解析：跳过前 16 行头部信息、UTF-8 BOM、列名映射（交易时间/交易类型/商品/金额/收支/当前状态）
  - 智能分类匹配：根据商品名称/备注关键词自动匹配一二级分类
  - 去重：相同日期+金额+备注的交易自动跳过
- ExportService 扩展: 本地 CSV 导出改为纯客户端实现（不依赖 gRPC）
- AuthService 扩展: 微信 OAuth、Apple Sign In

#### 客户端

- 导入页重构:
  - 支持三种模式：「支付宝账单」「微信账单」「通用 CSV」
  - 支付宝/微信模式: 自动解析、预览、一键导入（无需手动映射）
  - 通用 CSV: 保留现有字段映射流程
  - 本地解析 + 本地导入（不依赖服务端）
- 导出页修复:
  - 导出按钮改为纯本地导出（CSV），不走 gRPC
  - 分类筛选改为一级分类 + 二级分类层级选择
- 记账扩展: 币种选择器、自动换算人民币
- 交易详情扩展: 备注、标签、图片附件
...
- [ ] 支付宝账单导入: 选文件 → 自动解析 → 预览 → 一键导入
- [ ] 微信账单导入: 选文件 → 自动解析 → 预览 → 一键导入
- [ ] 通用 CSV 导入: 上传 → 预览 → 映射 → 导入
- [ ] 导出按钮可正常工作（本地 CSV，无需服务端）
- [ ] 导出分类筛选按一级/二级层级展示
